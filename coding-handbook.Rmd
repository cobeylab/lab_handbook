...
The so-called [Fundamental Theorem of Software Engineering](https://en.wikipedia.org/wiki/Fundamental_theorem_of_software_engineering) is: "We can solve any problem by introducing an extra level of indirection. (Except for the problem of too many levels of indirection)." (David J. Wheeler)
...
The benefits of functional programming are closely related to [modularity](#modularity). Functional programs are modular in that every function encapsulates some functionality and has a well-defined interface, the function signature.
...
Returning to the evolutionary algorithm example, a more modular version might look like this (adapted from [Rosetta Code]((https://www.rosettacode.org/wiki/Evolutionary_algorithm#Python)):
...
1. Clarity: How simply and understandably the concepts are presented.
2. Determinism: How well the starting state of the code, data, and environment are controlled so the user produces the same result as the researcher.
3. Interoperability: How well the code interfaces with other tools, and has a standard method for interacting with it.
...
Document for superficial users who will only run the runbook and leave, and users who might want to contribute to, or reuse your code. If you're writing Python, write [docstrings](https://peps.python.org/pep-0257/#abstract).
...
3. Include a runbook. It should start with data in the same format the user will receive and do any transformations. It should run tests, or give some indication that the run was successful.
...
6. Treat the repository as a published artifact. Respect publishing norms such as attribution and long-term access.
