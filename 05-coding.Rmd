...
### Transferring Data to MidwayR

Data is vulnerable "at rest" (on a local machine, or cloud storage) and "in transit" between machines over the internet or other network. It can also be recovered after it has been deleted from certain types of drive, and copies are retained on systems where the data was stored. Files in recycle bins, logs, backups, and swap files are examples of this, but there are many others. Online services (Slack, email systems, Github, messaging apps) where attachments appear to be limited to two parties, also expose data to the company that owns the service, and their internal logs, backups, third-party services et cetera.

It is important, therefore, to use secure systems even for temporary storage, or transferring files to MidwayR. Documentation on transferring files is available in the [MidwayR User Guide](https://midwayr-docs.rcc.uchicago.edu/data-transfer/).
