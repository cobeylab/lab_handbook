...
**Midway2**: [https://docs.rcc.uchicago.edu/](https://docs.rcc.uchicago.edu/)
...
[https://docs.rcc.uchicago.edu/101/connecting/](https://docs.rcc.uchicago.edu/101/connecting/)
...
[https://docs.rcc.uchicago.edu/101/connecting/#connecting-with-thinlinc](https://docs.rcc.uchicago.edu/101/connecting/#connecting-with-thinlinc)
...
[https://docs.rcc.uchicago.edu/software/](https://docs.rcc.uchicago.edu/software/)
...
#### Protected health information (PHI) and Midway {#phi-midway}
...
In practice, even when exact dates are necessary for the analyses, the dates themselves don't need to be on the cluster. It's easy to transform dates so they are expressed as a number of days since a reference date (the beginning of the study, the date when participants received an intervention, etc). As long as the reference date is not included, the numbers of days themselves can be kept on Midway2/Midway3. Similarly, when dates are used to determine the order of events (e.g., did an infection happen before this sample was collected?), keeping the order (e.g., `TRUE` or `FALSE`) without the actual dates on Midway2/Midway3 will be safe and sufficient for the analyses. Transforming and removing dates should be done in a device that is approved to store and analyze PHI. It should be done automatically by the code that prepares data for the analyses, never by hand.
...
```{r private-nodes, out.width='100%', echo=FALSE, fig.show='hold', fig.align="center", fig.cap='Node list with status September 2025'}
knitr::include_graphics(rep("images/private-nodes-Sept2025b.png"))
```
...
It is important, therefore, to use secure systems even for temporary storage of PHI, or when transferring files to collaborators. This excludes all apps and methods mentioned above.

The easiest way to safely store and transfer data containing PHI is to use [UChicago Box](https://uchicago.service-now.com/services?id=kb_article&sysparm_article=KB06000647). While you can upload and download files using the web interface, it's better to download the Box desktop app. The app gives you access to your Box folder via a path on your machine (`~/Box/`, if you're on a Mac). With the app installed, you should set up the following system for storing and sharing PHI:

- Create a directory for the project inside your Box folder.
- Initialize a git repository in that directory. You can do that by moving to the directory in the terminal (e.g. `cd ~/Box/project_X_data/`) and running `git init`. *Do not link this repo to Github*. Remember: Box is approved to store PHI, Github isn't. You can nonetheless version control the data, and the `.git/` folder will be backed up to Box like everything else in the folder.
- You can use the same Box folder for both storing and transferring data. Because the other person often shouldn't have access to the full data, you can create transfer folders inside the master project folder. On the Box website, you can authorize specific people to access just the transfer folder, and you can create multiple transfer folders for different people as needed. Do not use unrestricted links to share PHI.
- When you receive data, move it from the transfer folder to its permanent location, then stage and commit the files to the git repository. If you receive new versions of the same data, version control makes it safe to delete or overwrite previous versions. It's a good idea to keep READMEs in the data folder and its internal folders. Version control them too.
- Though git makes accidental changes to the data easy to detect and fix, you can guard against accidental edits (e.g. an unintended save in your editor) by making the files read-only (`chmod 444`). Note that this does not prevent git operations or a forced `rm` from changing the files.
- Because you can point to the data using the path to the mounted box folder (`~/Box/`), you don't need to keep copies of the data anywhere else on your machine. For instance, when you're developing code in a local copy of your code repo, you don't need to copy data from its permanent storage on Box into a `data/` folder next to where your code is. Doing so is bad because 1) you might accidentally push the copied data to GitHub, and 2) you need to make a fresh copy of the data whenever it's been updated on Box, so you risk developing code for or drawing conclusions from outdated data.
- See [PHI and Midway](#phi-midway) for how to transform and remove PHI before transferring data to Midway2/Midway3.
- CrashPlan Pro should be set up to exclude any PHI data directories, including the local Box mount.
- Claude should not be given access to PHI, including the local Box mount. If it tries to go there, the Box app will probably ask for your permission, which you should not give. You can explicitly prohibit Claude from accessing the Box mount by adding the following to your `settings.json` file:
...
PHI generally needs to be deleted after the conclusion of a study according to a Data Use Agreement, or IRB protocol. There's no official secure deletion application for individual files. However, [CCleaner](https://www.ccleaner.com/) and similar programs can help with the process on local machines. Whole-disk deletion should be coordinated with the [BSD Information Security Office](https://security.bsd.uchicago.edu/).

If data is exposed, lost or stolen, it should be reported according to the corresponding protocol or agreement.

Methods for transferring files to MidwayR are available in the [MidwayR User Guide](https://midwayr-docs.rcc.uchicago.edu/data-transfer/).
